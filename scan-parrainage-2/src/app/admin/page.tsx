...
export default function AdminPage() {
  ...
  // Algo Gale-Shapley simplifié (Stable Marriage)
  const runMatching = useCallback(() => {
    ...
    // Préférences parrains (du classement)
    parrainsList.forEach((p) => {
      const pc = getParrainClassements(p.id)
      prefs[p.id] = [1, 2, 3].map((pos) => pc[pos]?.id).filter(Boolean) as string[]
    })

    // Préférences bizuts (inverses : plus un bizut a de votes #1, plus il est "exigeant")
    // On inverse : les bizuts préfèrent les parrains qui les ont mis haut
    bizutsList.forEach((b) => {
      const bStats = stats.find((s) => s.bizut_id === b.id)
      // Tous les parrains classés par "score" (position inverse)
    ...
    // Gale-Shapley : parrains proposent
    const freeParrains = new Set(parrainsList.map((p) => p.id))
    const matches: Record<string, string | null> = {} // bizutId -> parrainId
    const parrainMatches: Record<string, string | null> = {} // parrainId -> bizutId
    ...
    while (freeParrains.size > 0 && iter < 1000) {
      ...
      if (!matched) freeParrains.delete(pId) // Aucun choix dispo
    }
